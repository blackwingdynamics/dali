...
workspace-check:
    cargo check --workspace --exclude {{kernel_cartridge}}

# Check the embedded kernel target.
kernel-check board="f405":
    cargo check -p {{kernel_cartridge}} --no-default-features --features {{ if board == "f405" { "stm32f405" } else { error("Unsupported board. Use f405.") } }} --bin {{kernel_binary}} --target {{target}}
...
clippy:
    cargo clippy --workspace --all-targets --exclude dali-kernel --exclude {{kernel_cartridge}} -- -D warnings

# Run kernel-target Clippy with warnings denied.
kernel-clippy board="f405":
    cargo clippy -p {{kernel_cartridge}} --no-default-features --features {{ if board == "f405" { "stm32f405" } else { error("Unsupported board. Use f405.") } }} --target {{target}} --bin {{kernel_binary}} -- -D warnings
...
build board="f405":
    cargo build -p {{kernel_cartridge}} --no-default-features --features {{ if board == "f405" { "stm32f405" } else { error("Unsupported board. Use f405.") } }} --bin {{kernel_binary}} --target {{target}}

# Run the kernel in Renode. Requires Renode and uses the STM32F4 reference model.
simulate board="f405":
    cargo build -p {{kernel_cartridge}} --no-default-features --features stm32f405 --bin {{kernel_binary}} --target {{target}}
    renode --console --disable-gui {{renode_script}}

# Convert the kernel ELF to a raw binary. Requires cargo-binutils and llvm-tools.
bin board="f405":
    just build {{board}}
    cargo objcopy -p {{kernel_cartridge}} --no-default-features --features stm32f405 --target {{target}} --bin {{kernel_binary}} -- -O binary {{f405_kernel_bin}}

# Build the native demo payload for the documented application SRAM region.
app-build:
    cargo build -p {{app_cartridge}} --features embedded-payload --target {{target}}
    cargo objcopy -p {{app_cartridge}} --features embedded-payload --target {{target}} --bin {{app_binary}} -- -O binary {{app_payload}}

# Assemble the native demo payload into a contract-valid AMRN cartridge.
cartridge-hello: app-build
    cargo run -p dali-cli --bin dali -- cartridge --input {{app_payload}} --output {{app_cartridge_file}} --entry-offset {{app_entry_offset}}

...
pub(super) struct PackageManifest {
    ...
    pub(super) package_version: BoundedText<{ dali_metadata::MAX_PACKAGE_VERSION_BYTES }>,
    pub(super) minimum_kernel_version: BoundedText<{ dali_metadata::MAX_PACKAGE_VERSION_BYTES }>,
    pub(super) package_version_parts: dali_amrn::v4::Version,
    pub(super) minimum_kernel_version_parts: dali_amrn::v4::Version,
    pub(super) required_services: u32,
    pub(super) slot_id: u8,
}

pub(super) fn parse(path: &std::path::Path) -> Result<PackageManifest, String> {
    ...
    let package_version_value = value(&contents, "version")?;
    let package_version_parts = version(&package_version_value, "version")?;
    let package_version = common::bounded::<{ dali_metadata::MAX_PACKAGE_VERSION_BYTES }>(
    ...
    let minimum_kernel_version = common::bounded::<{ dali_metadata::MAX_PACKAGE_VERSION_BYTES }>(
        &minimum_kernel_version_value,
        "minimum_kernel_version",
    )?;
    let package_id = common::parse_hex::<16>(&value(&contents, "package_id")?, "package_id")?;
    ...
    if abi_version != u32::from(dali_amrn::v3::ABI_VERSION) {
        return Err(format!(
            "only ABI {} packages are supported",
...
mod tests {
    ...
    fn parses_identity_and_slot_fields() {
        let path = std::env::temp_dir().join(format!(
            "dali-register-package-manifest-{}",
            std::process::id()
        ));
        fs::write(
            &path,

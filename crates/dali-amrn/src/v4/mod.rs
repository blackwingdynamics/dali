//! AMRN format version 4 identity metadata for ABI v3 cartridges.
...
/// Length of the opaque cartridge identity in bytes.
pub const CARTRIDGE_ID_LENGTH: usize = 16;
...
/// Offset of the stable cartridge identity in the fixed header.
pub const CARTRIDGE_ID_OFFSET: usize = 80;
/// Offset of the manifest-owned slot identifier in the extension header.
pub const SLOT_ID_OFFSET: usize = 112;
/// Offset of the cartridge CRC32 field in the extension header.
pub const CARTRIDGE_CRC32_OFFSET: usize = 116;
/// Offset where the cartridge CRC32 input resumes after the reserved header tail.
...
/// Identity and compatibility metadata carried by one v4 cartridge.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct Metadata {
    /// Stable opaque application identity.
    pub cartridge_id: [u8; CARTRIDGE_ID_LENGTH],
    /// Application cartridge version.
    pub cartridge_version: Version,
...
/// Input image and metadata used to construct an AMRN v4 cartridge.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct Image<'a> {
...
pub struct Header {
    ...
    pub image: v3::Header,
    /// Cartridge identity and compatibility metadata.
    pub metadata: Metadata,
    /// Integrity checksum covering the v3 payload and v4 metadata.
    pub cartridge_crc32: u32,
}

/// A validated v4 cartridge view into caller-owned bytes.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
pub enum Error {
    ...
    CrcMismatch,
    /// The output buffer cannot contain the encoded cartridge.
    OutputTooSmall,
}
...
pub fn encode(image: Image<'_>, contract: v3::Contract, output: &mut [u8]) -> Result<usize, Error> {
    if image.metadata.cartridge_id.iter().all(|byte| *byte == 0) {
    ...
    let cartridge_crc = cartridge_checksum(output);
    write_u32(output, CARTRIDGE_CRC32_OFFSET, cartridge_crc);
    Ok(cartridge_size)
}

/// Parses and validates a v4 cartridge against a target slot contract.
pub fn parse<'a>(cartridge: &'a [u8], contract: v3::Contract) -> Result<Cartridge<'a>, Error> {
    ...
    let data_end = code_end
        ...
        .ok_or(Error::InvalidPayload)?;
    let cartridge_end = data_end
        .checked_add(table_size)
        .ok_or(Error::InvalidPayload)?;
    if header.image.relocation_offset as usize != data_end || cartridge.len() != cartridge_end {
        return Err(Error::InvalidPayload);
    }
    let code = &cartridge[HEADER_SIZE..code_end];
    let initialized_data = &cartridge[code_end..data_end];
    let relocation_bytes = &cartridge[data_end..cartridge_end];
    if payload_checksum(code, initialized_data, relocation_bytes) != header.image.crc32
        || cartridge_checksum(cartridge) != header.cartridge_crc32
    {
...
pub fn parse_header(bytes: &[u8], contract: v3::Contract) -> Result<Header, Error> {
    ...
    let cartridge_id = read_array::<CARTRIDGE_ID_LENGTH>(bytes, CARTRIDGE_ID_OFFSET);
    ...
    Ok(Header {
        ...
        metadata: Metadata {
            cartridge_id,
            cartridge_version: read_version(bytes, CARTRIDGE_VERSION_OFFSET),
        ...
        cartridge_crc32: read_u32(bytes, CARTRIDGE_CRC32_OFFSET),
    })
}
...
fn cartridge_checksum(cartridge: &[u8]) -> u32 {
    let mut checksum = Crc32::new();
    checksum.update(&cartridge[RESERVED_BYTES_OFFSET..]);
    checksum.update(&cartridge[..CARTRIDGE_CRC32_OFFSET]);
    checksum.finish()
}
...
fn write_header(output: &mut [u8], image: v3::Header, metadata: Metadata) {
    ...
    output[CARTRIDGE_ID_OFFSET..CARTRIDGE_ID_OFFSET + metadata.cartridge_id.len()]

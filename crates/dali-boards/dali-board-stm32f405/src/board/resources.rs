...
use dali_kernel_api::WatchdogBackend;
...
pub struct Board {
    ...
    pub(super) watchdog: Option<crate::F405Watchdog>,
...
impl Board {
    ...
    pub fn reset_cause(&self) -> dali_kernel_api::ResetCause {
        self.watchdog
            .as_ref()
            .map_or(dali_kernel_api::ResetCause::Unknown, |watchdog| {
                watchdog.reset_cause()
            })
    }

    /// Transfers the unarmed watchdog backend to the kernel runtime owner.
    pub fn take_watchdog(&mut self) -> Option<crate::F405Watchdog> {
        self.watchdog.take()
    }
}

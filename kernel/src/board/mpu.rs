...
mod tests {
    ...
    fn rejects_non_power_of_two_or_misaligned_regions() {
        assert!(
            super::MpuRegion::new(
                MISALIGNED_BASE,
                super::MINIMUM_REGION_BYTES,
                super::MpuAccess::PrivilegedOnly,
                super::MpuExecution::Never
            )
            .is_none()
        );
        assert!(
    ...
    fn describes_the_manifest_memory_boundaries() {
        ...
        assert_eq!(layout.kernel.access, super::MpuAccess::PrivilegedOnly);
        assert!(layout.application_single_region.is_none());
        assert_eq!(layout.application_code.access, super::MpuAccess::ReadOnly);
        assert_eq!(
            layout.application_data.execution,
            super::MpuExecution::Never
        );
        assert_eq!(layout.peripherals.access, super::MpuAccess::PrivilegedOnly);
        assert_eq!(layout.peripherals.memory_type, super::MpuMemoryType::Device);
    }
    ...
    fn encodes_application_permissions_and_execution_policy() {
        let Some(code) = super::MpuRegion::new(
            0x2000_8000,
            32_768,
            super::MpuAccess::ReadOnly,
            super::MpuExecution::Allowed,
        ) else {
            return;
        };
        let Some(data) = super::MpuRegion::new(
            0x2001_0000,
            32_768,
            super::MpuAccess::ReadWrite,
            super::MpuExecution::Never,
        ) else {
            return;
        };

        assert_eq!(code.rasr_bits() & super::AP_READ_ONLY, super::AP_READ_ONLY);
        assert_eq!(code.rasr_bits() & super::RASR_XN, 0);
        assert_eq!(code.rasr_bits() & super::RASR_ENABLE, super::RASR_ENABLE);
        assert_eq!(
            data.rasr_bits() & super::AP_READ_WRITE,
            super::AP_READ_WRITE
        );
        assert_eq!(data.rasr_bits() & super::RASR_XN, super::RASR_XN);
    }
}

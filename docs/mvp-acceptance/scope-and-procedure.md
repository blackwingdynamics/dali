...
The MVP passes only when the kernel reads and executes an independently built `.amrn` cartridge from an SD card on the reference board.
...
- cartridge authenticity or digital signatures;
...
- independently built `.amrn` cartridge; `hello.amrn` is the reference
  cartridge name;
- cartridge built for `thumbv7em-none-eabihf`;
- cartridge load address: `0x20008000`;
- cartridge payload size no greater than 64 KiB;
- cartridge format version: `1`;
- cartridge target ID: `0x02` (`STM32F405RGT6`);
- cartridge ABI version: `2`;
- valid CRC32 over the payload.

The cartridge must use the documented entry ABI:
...
2. Copy exactly one test cartridge to the card root.
3. Use any valid `.amrn` filename; `hello.amrn` is only the reference
   cartridge name.
4. Safely eject the card.
5. Record the actual cartridge filename, size, format version, load address,
   entry offset, and CRC32.

The MVP does not require cartridge installation, cartridge deletion, or hot swap. The card is inserted before reset and remains unchanged during the test.
...
6. Confirm the kernel heartbeat appears before cartridge execution.
7. Confirm the SD card is initialized and the root cartridge is discovered.
...
After the loader validates the cartridge and transfers control, the kernel stops
...
- [x] Exactly one root `.amrn` cartridge is discovered.
...
- the cartridge is not discovered;
- malformed cartridge data causes a panic or uncontrolled jump;
...
Kernel revision:
Application cartridge revision:
Cartridge size:
Cartridge CRC32:
Observed log:

...
The application ABI is independent from the AMRN container format. A cartridge may have a valid header and checksum while still being incompatible with a kernel ABI.
...
The cartridge format carries the ABI version at header offset `0x18`.
...
single-region linker contract. ABI v3 cartridges must therefore be rejected by
...
The ABI v3 cartridge contract uses AMRN format version `2`. Format version `2`
...
to accept or emit ABI v3 cartridges.

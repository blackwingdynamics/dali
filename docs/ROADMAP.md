...
7. **Harden release and CI.** [x] Add the strict feature-matrix
...
3. **Package authenticity** — [in progress] define a bounded signature
   ...
   durable target installation remains pending; kernel wiring is tracked below.
   ...
   [x] Define the board-agnostic package-free `Recovery` trust-store payload
   schema with bounded Binary v2 references for root, timestamp, snapshot,
   targets, revocations, and delegations; durable payload installation and
   F405 boot wiring remain pending.

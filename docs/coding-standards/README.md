# Dali OS Coding Standards

This file is the stable entry point and navigation index for the coding
standards documentation. The files below contain its topic-oriented sections.

## Prepared sections

- [Language, hardcoding, and discipline](language-hardcoding-and-discipline.md)
- [Additional constraints](additional-constraints.md)
- [Comments and documentation](comments-and-documentation.md)
- [Safety, errors, and realtime](safety-errors-and-realtime.md)
- [Modules, naming, and logging](modules-naming-and-logging.md)
- [Testing, review, and prohibited practices](testing-review-and-prohibited-practices.md)
- [Engineering review checklist](review-checklist.md)
- [Terminology and documentation style](terminology-and-style.md)

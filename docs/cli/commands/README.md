# CLI Command Documentation

This directory contains the command-level syntax and behavior references for
the Dali CLI.

## Application commands

- [App build](app-build.md)
- [App init](app-init.md)
- [App new](app-new.md)
- [App package](app-package.md)
- [Package](package.md)
- [Inspect](inspect.md)

## Device and target commands

- [Device attach](device-attach.md)
- [Device console](device-console.md)
- [Device flash](device-flash.md)
- [Device info](device-info.md)
- [Doctor](doctor.md)
- [Target info](target-info.md)
- [Target list](target-list.md)
- [Target scaffold](target-scaffold.md)

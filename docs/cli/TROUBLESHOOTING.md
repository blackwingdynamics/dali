...
Confirm that the path exists and points to the intended payload or cartridge.
...
rename a Rust source file or raw payload to .amrn; create the cartridge with
dali cartridge.

## CRC mismatch

Rebuild and cartridge the payload again. Do not reuse a cartridge after changing
the payload, entry metadata, or build profile.

## Hardware logs are missing

Successful CLI inspection proves only cartridge validity. Check the separate
kernel flash, SD-card, USB CDC, and MVP acceptance procedures.

## Cargo cartridge versus executable name

The Cargo cartridge is dali-cli, while the installed executable is dali. Use

...
The current cartridge profile is AMRN format version 1, STM32F405 target
...
the CLI does not build v3 cartridges, and the kernel does not execute them yet.

Build and cartridge commands enforce the target manifest capabilities before
constructing an application cartridge. Isolation ABI builds require declared MPU
support, while AMRN relocation format builds require declared relocation
support.

## Breaking changes

Changes to cartridge bytes, validation rules, output semantics, or exit codes

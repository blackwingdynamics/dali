...
The manifest-owned slot table now drives the application linker, cartridge
...
"slot1"` in its application manifest, and produces a format 3 cartridge whose
code/data load addresses target slot 1. The CLI can build and inspect this
cartridge on the host. Slot 1 execution on the F405 remains a separate hardware
acceptance step.

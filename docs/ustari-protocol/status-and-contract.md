...
Ustari is the planned binary control, telemetry, and cartridge-delivery protocol
...
Ustari is separate from AMRN and cartridge signing:

- AMRN signatures establish whether a cartridge is trusted and installable.
- An Ustari session establishes whether a peer may issue a live command.
- A cartridge signing key is not an Ustari session key.
- A transport connection is not an authenticated session.

## 2. Goals and non-goals

Ustari provides a bounded command, telemetry, and cartridge-delivery protocol
...
7. Safe cartridge transfer with atomic activation and rollback.
...
- Ustari does not replace AMRN validation, cartridge signatures, Trust Store
...
- **Device** — the Dali OS endpoint. It owns authorization, cartridge policy,
